...
function FotoCard({
  ...
  return (
    <button
      onClick={onClick}
      className="group relative aspect-[4/5] w-full overflow-hidden rounded-xl bg-gray-200 focus:outline-none focus:ring-2 focus:ring-[#C9A84C] focus:ring-offset-2 hover:shadow-xl transition-shadow duration-300"
    ...
    >
      ...
      <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-black/0 to-black/0 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
        ...
        <div className="absolute inset-0 flex items-center justify-center">
          <div className="w-11 h-11 rounded-full bg-white/20 backdrop-blur-sm flex items-center justify-center translate-y-2 group-hover:translate-y-0 transition-transform duration-300">
            <ZoomIn className="w-5 h-5 text-white" />
          </div>
        </div>
        {/* Label kategori */}
        <div className="absolute bottom-3 left-3 translate-y-2 group-hover:translate-y-0 transition-transform duration-300">
          <span className="inline-block bg-[#C9A84C] text-white text-xs font-semibold px-2.5 py-1 rounded-full capitalize">
            {KATEGORI_LABEL[item.kategori as KategoriSesi]}
          </span>
        </div>
      </div>
